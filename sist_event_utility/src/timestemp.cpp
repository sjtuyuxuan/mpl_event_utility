...
int main (int argc, char* argv[]) {
  ...
  std::string event_left_topic, event_right_topic, rgb_left_topic, rgb_right_topic,
      imu_topic, kinect_depth_topic, kinect_rgb_topic, lidar_topic;
  ...
  if (gt) {
    ...
    if (!gt_file.is_open()) ROS_WARN("GT Load Unsuccess");
    else ROS_INFO("Start load GT from csv ...");
    std::string line;
    u_int64_t index = 0;
    ros::Time gt_time(0);
    for (int i = 0; i < 7; ++i) std::getline(gt_file, line);
  ...
  foreach(rosbag::MessageInstance const m, view) {
    if (event_left && m.getTopic() == event_left_topic) event_left_buffer.emplace_back(m.instantiate<sist_event_utility::EventArray>());
    if (event_right && m.getTopic() == event_right_topic) event_right_buffer.emplace_back(m.instantiate<sist_event_utility::EventArray>());
    ...
    if (kinect_depth && m.getTopic() == kinect_depth_topic) kinect_buffer.emplace_back(m.instantiate<sensor_msgs::Image>());
    if (kinect_rgb && m.getTopic() == kinect_rgb_topic) kinect_buffer_rgb.emplace_back(m.instantiate<sensor_msgs::Image>());
    ...
    for (int i = 1 ; i < kinect_buffer_rgb.size(); ++i) {
      if (kinect_buffer_rgb[i]->header.stamp.toSec() - temp.toSec() > 0.053) {
        ...
        if (i != kinect_buffer_rgb.size() - 1) {
          if (kinect_buffer_rgb[i+1]->header.stamp.toSec() - kinect_buffer_rgb[i]->header.stamp.toSec() < 0.015) {
            --lose_frame;
      ...
      kinect_buffer_rgb[i]->header.stamp.fromNSec((kinect_buffer_rgb_processed.size() + lose_frame) * 33333333);
  ...
  time_now.emplace_back(event_right ? event_right_buffer.front()->header.stamp.toNSec() : UINT64_MAX);
    ...
    switch (static_cast<int>(std::distance(time_now.begin(), early))) {
      ...
      case 6:
        if (!lidar_buffer.empty()) {
          ...
          time_now[6] = lidar_buffer.front()->header.stamp.toNSec() + 100000000 - start_time.toNSec();
      ...
      case 7:
        if (!kinect_buffer_rgb_processed.empty()) {
          ...
          bag_out.write(out_kinect_rgb_topic,
              kinect_buffer_rgb_processed.front()->header.stamp, kinect_buffer_rgb_processed.front());
      ...
      case 8:
        if (!gt_buffer.empty()) {
          ...
          bag_out.write(out_gt_topic,
              gt_buffer.front()->header.stamp, gt_buffer.front());
          gt_buffer.pop_front();
        } else {
